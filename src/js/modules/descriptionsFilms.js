...
function descriptionsFilms() {
  ...
  Content.addEventListener(`click`, (event) => {
    ...
    if (
      target.className == `FigureOfLeftNewFilm` ||
      target.className == `FigureOfLeftSerials`
    ) {
      hideAllWorkSpace();
      ...
      if (target.className == `FigureOfLeftNewFilm`) {
        getResource(" http://localhost:3000/JSONObjectForNewFilm").then(
          (data) => {
            data.forEach(
              ({ moveFilm, genre, title, descr, IMDb, kinopoisk }) => {
        ...
        getResource(" http://localhost:3000/JSONObjectForSerilas").then(
          (data) => {
            data.forEach(
              ({ moveFilm, genre, title, descr, IMDb, kinopoisk }) => {
                if (moveFilm === newFigireOfFilm.join("")) {
